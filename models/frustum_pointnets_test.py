...
def get_model(point_cloud, one_hot_vec, is_training, bn_decay=None):
   ...
   #############Invariance transformation Net###########################################
    ...
    #invariance_transformation_net=Invariance_Transformation_Net(point_cloud=point_cloud_xyz,features=features_initial,is_training=is_training,invarians_trans_param=invariants_trans_param_7_layer).layer_fts[-1]
    
    #3 layer with pointcnn parameter###
    
    #invariance_transformation_net=Invariance_Transformation_Net(point_cloud=point_cloud_xyz,features=features_initial,is_training=is_training,invarians_trans_param=invariants_trans_param).layer_fts[-1]
    


    #print("invariance_transformation_net",tf.shape(invariance_transformation_net))
    
    
    #print("invariance_transformation_net",invariance_transformation_net.get_shape())
    ...
    edge_feature_edge = get_edge_feature(edge_net, nn_idx=nn_idx,k=k)
    ...
    with tf.variable_scope('feature_transform_net') as sc:
        transform = input_transform_net(net3,is_training,bn_decay,K=3)
    
    print("feature_transform_shape",transform.get_shape())
    
    feature_transformed = tf.matmul(point_cloud_xyz,transform)
    print("feature_transformed",feature_transformed.get_shape())

    point_cloud_concat=tf.concat([point_cloud,feature_transformed],axis=-1)
    ...
    #point_cloud_invari= tf.concat([point_cloud_concat,invariance_transformation_net],axis=-1)
